...
courseRouter.post(
    ...
    async (req, res) => {
      ...
      const { course_name, price, instructor_name, description, category } = req.body;
      const user_id = req.user.id;
  
      if (!course_name || !price || !description || !category) {
        return res.status(400).json({ message: "Course name, price, description, and category are required" });
      }
      ...
      try {
        ...
        const [result] = await pool.query(
          "INSERT INTO courses (user_id, course_name, course_image, price, instructor_name, curriculum_pdf, description, category) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
          [user_id, course_name, courseImageUrl, price, instructor_name, curriculumPdfUrl, description, category]
        );
...
courseRouter.get("/registered-courses",verifyToken, async (req, res) => {
    ...
    try {
        const [registeredCourses] = await pool.query(
            "SELECT c.* FROM courses c INNER JOIN registered_courses rc ON c.id = rc.course_id WHERE rc.user_id = ?",
            [userId]
        );
...
courseRouter.get("/non-registered-courses", verifyToken, async (req, res) => {
    ...
    try {
        const [nonRegisteredCourses] = await pool.query(
            `SELECT c.* 
             FROM courses c
             LEFT JOIN registered_courses rc ON c.id = rc.course_id AND rc.user_id = ?
             WHERE rc.course_id IS NULL`,
            [userId]
        );
